...
export default function OwnerBarrierForm() {
  ...
  const [ownerIdDocPicTypes, setownerIdDocPicTypes] = useState<
    OwnerIdDocPicType[]
  >([]);
  const [ownerDocPdfTypes, setownerDocPdfTypes] = useState<
    OwnerResidentialDocPdfType[]
  >([]);
      ...
      if (response.status === 200) {
        ...
        setFormData((prev) => ({
          ...
          phone: userData.phoneno
            ? sanitizePhone(userData.phoneno.replace("+1", ""))
            : "",
        }));
  ...
  useEffect(() => {
    if (formData.canadian_provinceid) {
      ...
      const canadianprov = canadianProvinces.find(
        (opt) =>
          String(opt.canadian_province_id) == formData.canadian_provinceid
      );
    ...
    if (formData.canadian_cityid) {
      var canadiancities = canadianCities.find(
        (opt) => String(opt.canadian_city_id) == formData.canadian_cityid
      );
      if (canadiancities) {
        setFormData((prev) => ({
          ...prev,
    ...
    switch (field) {
      case "legalName": {
        ...
        if (!v) msg = "Legal name is required.";
          ...
          msg = "Letters, spaces, and hyphens only. Max 50 characters.";
        break;
      }
      case "displayName": {
        const v = String(value);
        if (v && !displayNameRegex.test(v))
          msg = "Letters and spaces only. Max 15 characters.";
        break;
      }
      case "email":
  ...
  const validateAll = () => {
    ...
    if (!filesOk) {
      setErrors((e) => ({
        ...
        ownershipDoc: !uploadedFiles.ownershipDoc
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (uploadedFiles.ownershipDoc?.file) {
        formDataToSend.append(
          "residentialdocpdf",
          uploadedFiles.ownershipDoc.file
        );
      }
    ...
    <div className="space-y-8">
      ...
      <form noValidate className="space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="space-y-2">
          ...
          />
          {errors.legalName && (
            <p className="text-xs text-red-500 mt-1">{errors.legalName}</p>
          )}
        </div>
            ...
            <Select
              ...
              value={
                canadianProvinces
                  ...
                  .find(
                    (opt) => opt.value === String(formData.canadian_provinceid)
                  ) || null
              }
              onChange={(opt: any) => {
                setField(
            ...
            <Select
              ...
              value={
                (canadianCities || [])
                  ...
                  .find(
                    (opt) => opt.value === String(formData.canadian_cityid)
                  ) || null
              }
              onChange={(opt: any) => {
                setField(
        ...
        <div className="space-y-6 pt-4 border-t border-gray-200">
          ...
          <div className="space-y-2">
            ...
            {errors.proofType && (
              <p className="text-xs text-red-500 mt-1">{errors.proofType}</p>
            )}
          </div>
        ...
        <div className="space-y-4 pt-4 border-t border-gray-200">
          <div className="flex items-start gap-3">
            ...
            >
              I confirm I am the lawful owner or authorized representative of
              this property. <span className="text-red-500">*</span>
            </Label>
          </div>
          {errors.agreeOwnership && (
            <p className="text-xs text-red-500 ml-7">{errors.agreeOwnership}</p>
          )}
