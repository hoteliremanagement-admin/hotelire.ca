...
export default function HotelDetailPage({ id }: { id: string }) {
    ...
    (raw.propertysafetyfeatures ?? []).forEach((a: any) => {
      if (a?.safetyfeatures?.safetyfeaturesname) {
        setPropertySafetyFeatures((prev) => [
          ...prev,
          {
            label: a.safetyfeatures?.safetyfeaturesname,
            icon: a.safetyfeatures?.icons,
          },
        ]);
      }
    });
    (raw.propertysharedspaces ?? []).forEach((a: any) => {
      if (a?.sharedspaces?.sharedspacesname) {
        setPropertySharedSpaces((prev) => [
          ...prev,
          {
            label: a.sharedspaces?.sharedspacesname,
            icon: a.sharedspaces?.icons,
          },
        ]);
      }
    });
    ...
    const fetchProperty = async () => {
      try {
        const res = await axios.get(
          `${baseUrl}/ownerProperty/getProperties/${id}`,
        ...
        const Details = res.data.properties.map((p: any) =>
          mapPropertyToDetail(p),
        );
        ...
        setLocation(
          Details[0].canadian_city_name +
            " , " +
            Details[0].canadian_province_name,
        );
      } catch (ex: any) {
  ...
  // const increaseRoomQty = (propertyroomid: number) => {
  //   const room = propertyDetail?.PropertyRoom.find(r => r.propertyroomid === propertyroomid);
  //   const maxAvailable = room?.available_count ?? 0;

  //   setRoomQtyMap((prev) => {
  //     const currentQty = prev[propertyroomid] || 0;
  //     // This line strictly prevents selecting more than what's available
  //     if (currentQty < maxAvailable) {
  //       return {
  //         ...prev
  //         , [propertyroomid]: currentQty + 1
  //       };
  //     }
  //     return prev;
  //   });
  ...
  const increaseRoomQty = (propertyroomid: number) => {
    const room = propertyDetail?.PropertyRoom.find(
      (r) => r.propertyroomid === propertyroomid,
    );
    const maxAvailable = room?.available_count ?? 0;
    const currentQty = roomQtyMap[propertyroomid] || 0;

    // Calculate how many rooms are already in the cart for this specific room type
    const cartItem = cart.find(
      (item) => item.propertyroomid === propertyroomid,
    );
    const inCartQty = cartItem ? cartItem.quantity : 0;

    // TOTAL SELECTED = (Quantity in cart) + (Quantity currently selected in the selector)
    const totalSelected = inCartQty + currentQty;
    ...
    if (totalSelected < maxAvailable) {
      ...
      toast.error(`Only ${maxAvailable} rooms available in total`);
    }
  };
  ...
  const removeFromCart = (propertyroomid: number) => {
    setCart((prev) =>
      prev.filter((item) => item.propertyroomid !== propertyroomid),
    );
  };
  ...
  const handleReserve = async () => {
    ...
    try {
      // Prepare payload for availability check
      const payload = {
      ...
      // ✅ If available, store booking summary in localStorage
      const nights = calculateNights();
      const summaryPayload = {
        property: {
          id: propertyDetail?.propertyid,
  ...
  const openRoomGallery = (images: string[], index: number) => {
  ...
  return (
    <div className="bg-white w-full flex flex-col min-h-screen ">
                ...
                <PopoverContent className="w-[280px] p-4" align="start">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p
                          className="font-semibold text-[#3F2C77] text-sm"
                          style={{ fontFamily: "Poppins, sans-serif" }}
                        >
                          Adults
                        </p>
      ...
      <div className="w-full px-4 md:px-8 lg:px-[103px] py-8 site-container">
        ...
        <p
          className="text-[#161717]-500 text-sm mb-2"
          style={{ fontFamily: "Inter, sans-serif" }}
          data-testid="text-hotel-subtitle"
        >
          {propertyDetail?.propertysubtitle}
        </p>
        <p
          className="text-#161717-600 text-sm mb-6"
          style={{ fontFamily: "Inter, sans-serif" }}
          data-testid="text-hotel-location"
        >
          {propertyDetail?.canadian_city_name},{" "}
          {propertyDetail?.canadian_province_name}
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-8">
              <div className="w-full h-[300px] md:h-[400px] bg-gray-200 rounded-lg">
                <img
                  ...
                  className="w-full h-[300px] md:h-[400px] rounded-lg object-cover cursor-pointer"
                  src={propertyDetail?.photo1_featured}
                  alt="featured"
                />
              </div>
                ...
                <div className="w-full h-[194px] bg-gray-200 rounded-lg">
                  {propertyDetail?.photo2 && (
                    <img
                      ...
                      className="w-full h-[194px] rounded-lg object-cover cursor-pointer"
                      src={propertyDetail?.photo2}
                      alt="featured"
                    />
                  )}
                </div>
            ...
            <section className="mb-8">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                <h2
                  className="text-[24px] font-bold text-[#3F2C77]"
                  style={{ fontFamily: "Poppins, sans-serif" }}
                >
                  Overview And Rules
                </h2>
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1 bg-[#3F2C77] text-white px-2 py-1 rounded">
                    <Star className="w-4 h-4 fill-current" />
                    <span className="font-bold text-sm">
                      {propertyDetail?.avgRating}
                    </span>
                  </div>
                </div>
              </div>
              <p
                className="text-[14px] text-[#161717]-700 leading-relaxed mb-4"
                style={{ fontFamily: "Inter, sans-serif" }}
              >
            ...
            <section className="mb-10">
              <h2
                className="text-[24px] font-bold text-[#3F2C77] mb-6"
                style={{ fontFamily: "Poppins, sans-serif" }}
              >
                Most popular services & amenities
              </h2>
                ...
                {PropertyAmenities.length > 0 ? (
                  PropertyAmenities.map((amenity, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <FontAwesomeIcon
                        icon={getIcon(amenity.icon) as IconDefinition}
                        className="w-5 h-5 text-[#161717]"
                      />
                      <span
                        className="text-[13px] text-[#161717]"
                        style={{ fontFamily: "Inter, sans-serif" }}
                      >
                        {amenity.label}
                      </span>
            ...
            <section className="mb-10">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {PropertySafetyFeatures.length > 0 ? (
                  PropertySafetyFeatures.map((feature, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <FontAwesomeIcon
                        icon={getIcon(feature.icon) as IconDefinition}
                        className="w-5 h-5 text-[#161717]"
                      />
                      <span
                        className="text-[13px] text-[#161717]"
            ...
            <section className="mb-10">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {PropertySharedSpaces.length > 0 ? (
                  PropertySharedSpaces.map((space, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <FontAwesomeIcon
                        icon={getIcon(space.icon) as IconDefinition}
                        className="w-5 h-5 text-[#161717]"
                      />
                      <span
                        className="text-[13px] text-[#161717]"
            ...
            <section className="mb-12">
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                ...
                <div className="hidden md:grid grid-cols-12 bg-[#FEC328] py-4 px-6">
                  <div
                    className="col-span-3 font-bold text-black text-sm"
                    style={{ fontFamily: "Poppins, sans-serif" }}
                  >
                    Room Image
                  </div>
                  ...
                  <div
                    className="col-span-3 font-bold text-black text-sm text-center"
                    style={{ fontFamily: "Poppins, sans-serif" }}
                  >
                    Price for per night
                  </div>
                ...
                {propertyDetail?.PropertyRoom.map((room) => {
                  ...
                  return (
                    <div
                      ...
                      className="border-b border-gray-200 p-4 md:p-6"
                    >
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                        ...
                        <div className="col-span-1 md:col-span-3">
                          <h1
                            className="font-semibold text-[#3F2C77] text-xs text-center mb-2"
                            style={{ fontFamily: "Poppins, sans-serif" }}
                          >
                            {room.roomtypename}
                          </h1>
                        ...
                        <div className="col-span-1 md:col-span-3 flex items-center justify-start md:justify-center gap-3 flex-wrap">
                          ...
                          <div className="flex items-center border border-gray-300 rounded">
                            <button
                              ...
                              onClick={() =>
                                decreaseRoomQty(room.propertyroomid)
                              }
                              disabled={tempQty <= 0}
                            ...
                            <span
                              className="px-4 py-2 text-sm font-medium border-x border-gray-300 min-w-[40px] text-center"
                              style={{ fontFamily: "Inter, sans-serif" }}
                            >
                            ...
                            <button
                              ...
                              onClick={() =>
                                increaseRoomQty(room.propertyroomid)
                              }
                          ...
                          <Button
                            variant="outline"
                            className="border-[#3F2C77] text-[#3F2C77] hover:bg-[#3F2C77] hover:text-white px-12  disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={tempQty === 0}
                            onClick={() =>
                              addToCart(
                                room.propertyroomid,
            ...
            {isRoomGalleryOpen && (
              <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
                ...
                {/* Image */}
                <img
                  src={currentRoomImages[currentRoomImageIndex]}
                  className="max-h-[90vh] max-w-[90vw] rounded-lg"
                  alt=""
                />
          ...
          <div className="lg:col-span-1">
            <Card className="sticky top-4 p-6">
              {/* Check in - Check out */}
              <div
                className="mb-6 cursor-pointer hover:bg-gray-50 rounded-lg p-2 transition"
                onClick={handleSidebarDateClick}
              >
                <h3
                ...
                <div
                  ...
                  style={{ fontFamily: "Inter, sans-serif" }}
                >
                  <CalendarIcon className="w-4 h-4 text-[#3F2C77]" />
                  <span>
                    {checkInDate && checkOutDate
                      ? `${format(checkInDate, "MMM dd yyyy")} - ${format(checkOutDate, "MMM dd yyyy")}`
                      : "Select dates"}
              ...
              <div
                className="mb-6 cursor-pointer hover:bg-gray-50 rounded-lg p-2 transition"
                onClick={handleSidebarGuestClick}
              >
                <h3
                  className="text-lg font-semibold text-gray-800 mb-2"
                  style={{ fontFamily: "Poppins, sans-serif" }}
